...
pub fn get_global_rep(statement: &GlobalStatement) -> String {
    let global_rep = match &statement.definition {
        ...
        sqparse::ast::GlobalDefinition::UntypedVar { name, initializer } => {
            ...
            format!("{ty}{}{}", name.value, get_var_initializer_rep(initializer))
        }
        sqparse::ast::GlobalDefinition::TypedVar(_) => todo!(),
        sqparse::ast::GlobalDefinition::Const(p) => get_const_rep(p),
